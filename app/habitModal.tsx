    ...
    return (
        <ModalScreen name="Habit Details">
            ...
            <View style={styles.container}>
                ...
                <Text>Logged Today: {habit?.isLoggedToday ? "Yes" : "No"}</Text>
                <Text>Current Streak: {current}</Text>
                <Text>Best Streak: {best}</Text>
                {habit?.rewards && habit.rewards.length > 0 && (
                    <View style={{ marginTop: 16 }}>
                        <Text style={{ fontSize: 18, fontWeight: "600" }}>Rewards:</Text>
                        {habit.rewards.map((reward) => (
                            <View key={reward.id} style={{ marginLeft: 16, marginTop: 8 }}>
                                <Text style={{ fontSize: 16 }}>{reward.name}</Text>
                                <Text style={{ fontSize: 14, color: "#555" }}>{reward.description}</Text>
                                <Text style={{ fontSize: 14, color: "#555" }}>Type: {reward.type}</Text>
                                <Text style={{ fontSize: 14, color: "#555" }}>Quantity: {reward.quantity}</Text>
                                <Text style={{ fontSize: 14, color: "#555" }}>Need {reward.requirements - current} log(s) to receive reward</Text>
                            </View>
                        ))}
                    </View>
                )}
